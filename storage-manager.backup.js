...
class StorageManager {
    ...
    setAnalysisData(data) {
        ...
        const analysisData = {
            ...
            amenities: {
                ...
                offices: data.amenities?.offices || 0,
                ...
                gas_stations: data.amenities?.gas_stations || data.amenities?.gasStations || 0,
                atms: data.amenities?.atms || 0,
                pharmacies: data.amenities?.pharmacies || 0,
                shopping_centers: data.amenities?.shopping_centers || data.amenities?.malls || 0,
                public_transport: data.amenities?.public_transport || 0,
                parking: data.amenities?.parking || 0,
    ...
    async createProfessionalPDF(allData) {
        ...
        // Land Use Analysis with Chart
        checkNewPage(100);
        currentY = await this.addLandUseAnalysis(doc, allData, margin, currentY, contentWidth);
        
        // Amenities Analysis with Chart
        checkNewPage(120);
        currentY = await this.addAmenitiesAnalysis(doc, allData, margin, currentY, contentWidth);
    ...
    async addLandUseAnalysis(doc, allData, margin, startY, contentWidth) {
        let currentY = startY;
        ...
        try {
            ...
            doc.setFont(undefined, 'bold');
            doc.setTextColor(0, 0, 0);
            doc.text('LAND USE ANALYSIS', margin + 3, currentY + 4);
            currentY += 15;
            
            const analysisData = allData.analysis || {};
            // Try multiple paths for land use data
            const landUse = analysisData.landUse || allData.landUse || {};
            const percentages = landUse.percentages || {};
            const counts = landUse.counts || {};
            
            console.log('🏗️ Land use data found:', landUse);
            
            // Create land use chart if we have data
            if (Object.keys(percentages).length > 0 || Object.keys(counts).length > 0) {
                try {
                    const chartCanvas = await this.createLandUseChart(landUse);
                    if (chartCanvas) {
                        // Add chart with better sizing
                        const imgData = chartCanvas.toDataURL('image/png');
                        const chartX = margin + (contentWidth - 150) / 2;
                        doc.addImage(imgData, 'PNG', chartX, currentY, 150, 90);
                        currentY += 100;
                    ...
                    console.warn('Chart creation failed, continuing without chart:', chartError);
                }
            }
            
            // Land use breakdown table with professional styling
            doc.setFillColor(248, 249, 250);
            doc.rect(margin, currentY - 3, contentWidth, 10, 'F');
            doc.setFont(undefined, 'bold');
            doc.setFontSize(12);
            doc.setTextColor(28, 102, 69);
            doc.text('LAND USE BREAKDOWN', margin + 2, currentY + 4);
            currentY += 18;

            // Professional table headers with proper fixed column widths
            doc.setFillColor(245, 245, 245);
            doc.rect(margin, currentY - 2, contentWidth, 8, 'F');
            doc.setDrawColor(220, 220, 220);
            doc.setLineWidth(0.5);
            doc.rect(margin, currentY - 2, contentWidth, 8, 'S');
            
            // Fixed column positions and widths
            ...
            const col4Width = contentWidth - (col4X - margin) - 5;
            
            doc.setFont(undefined, 'bold');
            doc.setFontSize(10);
            doc.setTextColor(0, 0, 0);
            doc.text('Land Use Type', col1X, currentY + 4);
            doc.text('Area (%)', col2X, currentY + 4);
            doc.text('Count', col3X, currentY + 4);
            doc.text('Assessment', col4X, currentY + 4);
            currentY += 12;
            
            // Land use data rows with proper text wrapping
            doc.setFont(undefined, 'normal');
            const landUseTypes = [
                { key: 'commercial', label: 'Commercial', assessment: 'High potential for fuel station' },
                { key: 'residential', label: 'Residential', assessment: 'Steady customer base' },
                { key: 'industrial', label: 'Industrial', assessment: 'Fleet vehicle demand' },
                { key: 'retail', label: 'Retail', assessment: 'High traffic area' },
                { key: 'mixed', label: 'Mixed Use', assessment: 'Diverse customer mix' },
                { key: 'office', label: 'Office', assessment: 'Business commuter traffic' }
            ];
            
            let rowIndex = 0;
            landUseTypes.forEach((type) => {
                const percentage = parseFloat(percentages[type.key]) || 0;
                const count = parseInt(counts[type.key]) || 0;
                ...
                if (percentage > 0 || count > 0) {
                    ...
                    // Land Use Type - truncate if too long
                    doc.setTextColor(0, 0, 0);
                    doc.setFont(undefined, 'normal');
                    doc.setFontSize(9);
                    const truncatedLabel = type.label.length > 12 ? type.label.substring(0, 12) + '...' : type.label;
                    doc.text(truncatedLabel, col1X, currentY + 4);
                    
                    // Percentage
                    doc.setFont(undefined, 'bold');
                    doc.text(`${percentage.toFixed(1)}%`, col2X, currentY + 4);
                    
                    // Count
                    doc.setFont(undefined, 'normal');
                    doc.text(count.toString(), col3X, currentY + 4);
                    
                    // Assessment - wrap text properly
                    doc.setTextColor(70, 70, 70);
                    doc.setFontSize(8);
                    const wrappedText = doc.splitTextToSize(type.assessment, col4Width - 5);
                    if (wrappedText.length > 1) {
                        // Multi-line text
                        doc.text(wrappedText[0], col4X, currentY + 2);
                        doc.text(wrappedText[1] || '', col4X, currentY + 6);
                        currentY += 12; // Extra space for multi-line
                    } else {
                        // Single line
                        doc.text(type.assessment, col4X, currentY + 4);
                        currentY += 10;
                    }
            ...
            // Enhanced Site Characteristics Summary
            currentY += 8;
            doc.setFillColor(248, 249, 250);
            doc.rect(margin, currentY - 3, contentWidth, 10, 'F');
            doc.setTextColor(28, 102, 69);
            doc.setFont(undefined, 'bold');
            doc.setFontSize(12);
            doc.text('SITE CHARACTERISTICS', margin + 2, currentY + 4);
            currentY += 15;
            
            const dominantType = analysisData.dominantLandUse || 'Mixed Use';
            const viabilityScore = Math.max(parseFloat(analysisData.commercialViability) || 65, 65);
            
            // Professional characteristics table with better column management
            ...
            const characteristics = [
                ...
                { 
                    ...
                    value: this.getZoningCompatibility(landUse)
                },
                { 
                    label: 'Development Potential:', 
                    value: this.getDevelopmentPotential(landUse)
                },
                { 
                    label: 'Traffic Generation:', 
                    value: this.getTrafficGeneration(landUse)
                }
            ];
            
            // Fixed positions for characteristics table
            const labelX = margin + 3;
            const valueX = margin + 80;
            const maxValueWidth = contentWidth - 85;
            ...
            characteristics.forEach((char, index) => {
                ...
                doc.setFont(undefined, 'normal');
                doc.setFontSize(9);
                doc.text(char.label, labelX, currentY + 4);
                
                // Value with text wrapping if needed
                doc.setTextColor(0, 0, 0);
                doc.setFont(undefined, 'bold');
                doc.setFontSize(9);
                
                // Check if value needs wrapping
                const wrappedValue = doc.splitTextToSize(char.value, maxValueWidth);
                if (wrappedValue.length > 1) {
                    doc.text(wrappedValue[0], valueX, currentY + 2);
                    if (wrappedValue[1]) {
                        doc.setFontSize(8);
                        doc.text(wrappedValue[1], valueX, currentY + 6);
                    }
                    currentY += 12;
                } else {
                    doc.text(char.value, valueX, currentY + 4);
                    currentY += 10;
                }
            });
            
        } catch (error) {
            console.error('Error in addLandUseAnalysis:', error);
            // Add error message to PDF
            doc.setTextColor(255, 0, 0);
            doc.setFontSize(10);
            doc.text('Land Use Analysis: Data processing error', margin, currentY);
            currentY += 10;
        }
    ...
    async addAmenitiesAnalysis(doc, allData, margin, startY, contentWidth) {
        let currentY = startY;
        ...
        doc.setFont(undefined, 'bold');
        doc.setTextColor(0, 0, 0);
        doc.text('AMENITIES & INFRASTRUCTURE ANALYSIS', margin + 3, currentY + 4);
        currentY += 15;
        
        const analysisData = allData.analysis || {};
        // Try multiple paths for amenities data
        const amenities = analysisData.amenities || allData.amenities || {};
        
        console.log('🏢 Amenities data found:', amenities);
        
        // Extract counts from both direct values and details arrays
        const getAmenityCount = (key) => {
            // First try direct count
            if (amenities[key] && typeof amenities[key] === 'number') {
                return amenities[key];
            }
            
            // Then try from details arrays
            if (amenities.details && amenities.details[key] && Array.isArray(amenities.details[key])) {
                return amenities.details[key].length;
            }
            
            // Try from counts object
            if (amenities.counts && amenities.counts[key]) {
                return parseInt(amenities.counts[key]) || 0;
            }
            
            // Try from summary object
            if (amenities.summary && amenities.summary[key]) {
                return parseInt(amenities.summary[key]) || 0;
            }
            
            return 0;
        };
        
        // Create a normalized amenities object with actual counts
        const normalizedAmenities = {
            schools: getAmenityCount('schools'),
            hospitals: getAmenityCount('hospitals'),
            restaurants: getAmenityCount('restaurants'),
            malls: getAmenityCount('malls'),
            banks: getAmenityCount('banks'),
            offices: getAmenityCount('offices'),
            hotels: getAmenityCount('hotels'),
            parks: getAmenityCount('parks'),
            gas_stations: getAmenityCount('fuelStations') || getAmenityCount('gasStations') || getAmenityCount('gas_stations'),
            shopping_centers: getAmenityCount('shopping_centers') || getAmenityCount('malls'),
            public_transport: getAmenityCount('public_transport') || getAmenityCount('busStations'),
            parking: getAmenityCount('parking')
        };
        
        console.log('🎯 Normalized amenities data:', normalizedAmenities);
        
        // Create amenities chart if we have data
        const chartTotalAmenities = Object.values(normalizedAmenities).reduce((sum, count) => sum + count, 0);
        if (chartTotalAmenities > 0) {
            try {
                const chartCanvas = await this.createAmenitiesChart(normalizedAmenities);
                if (chartCanvas) {
                    // Add chart with better sizing
                    const imgData = chartCanvas.toDataURL('image/png');
                    const chartX = margin + (contentWidth - 150) / 2;
                ...
                console.warn('Amenities chart creation failed, continuing without chart:', chartError);
            }
        }
        
        // Amenities summary table with consistent styling
        doc.setFillColor(248, 249, 250);
        doc.rect(margin, currentY - 3, contentWidth, 10, 'F');
        doc.setFont(undefined, 'bold');
        doc.setFontSize(12);
        doc.setTextColor(28, 102, 69);
        doc.text('AMENITIES SUMMARY', margin + 2, currentY + 4);
        currentY += 18;
        ...
        doc.setFontSize(10);
        doc.setTextColor(0, 0, 0);
        doc.text('Amenity Type', margin + 3, currentY + 4);
        doc.text('Count', margin + 100, currentY + 4);
        doc.text('Impact Level', margin + 140, currentY + 4);
        doc.text('Business Value', margin + 195, currentY + 4);
        currentY += 12;        // Amenities data
        doc.setFont(undefined, 'normal');
        const amenityCategories = [
            { key: 'schools', label: 'Educational Institutions', weight: 0.8, desc: 'Family traffic' },
            { key: 'hospitals', label: 'Healthcare Facilities', weight: 0.9, desc: 'Emergency traffic' },
            { key: 'restaurants', label: 'Restaurants & Cafes', weight: 0.7, desc: 'Convenience stops' },
            { key: 'malls', label: 'Shopping Centers', weight: 1.0, desc: 'High footfall' },
            { key: 'banks', label: 'Financial Services', weight: 0.6, desc: 'Regular visits' },
            { key: 'offices', label: 'Office Buildings', weight: 0.8, desc: 'Commuter traffic' },
            { key: 'hotels', label: 'Hotels & Lodging', weight: 0.7, desc: 'Tourist traffic' },
            { key: 'parks', label: 'Parks & Recreation', weight: 0.5, desc: 'Weekend traffic' }
        ];
        
        // Filter categories to show only those with counts > 0
        const categoriesWithData = amenityCategories.filter(category => {
            const count = normalizedAmenities[category.key] || 0;
            return count > 0;
        });
        ...
        if (categoriesWithData.length === 0) {
            ...
            doc.setFontSize(10);
            doc.setTextColor(108, 117, 125);
            doc.text('No significant amenities found in this area.', margin + 3, currentY + 5);
            currentY += 20;
        } else {
            let rowIndex = 0;
            categoriesWithData.forEach((category) => {
                ...
                // Calculate impact level with consistent styling
                let impactLevel = 'Low';
                if (count >= 20) {
                ...
                // Amenity type with consistent font
                doc.setTextColor(0, 0, 0);
                doc.setFont(undefined, 'normal');
                doc.setFontSize(10);
                doc.text(category.label, margin + 3, currentY + 4);
                
                // Count with emphasis
                doc.setFont(undefined, 'bold');
                doc.setTextColor(0, 0, 0);
                doc.text(count.toString(), margin + 100, currentY + 4);
                
                // Impact level with consistent styling
                doc.setFont(undefined, 'bold');
                doc.setTextColor(0, 0, 0);
                doc.text(impactLevel, margin + 140, currentY + 4);
                
                // Business value description
                doc.setFont(undefined, 'normal');
                doc.setFontSize(9);
                doc.setTextColor(70, 70, 70);
                doc.text(category.desc, margin + 195, currentY + 4);
        ...
        // Infrastructure assessment with professional styling
        doc.setFillColor(248, 249, 250);
        doc.rect(margin, currentY - 3, contentWidth, 10, 'F');
        doc.setTextColor(28, 102, 69);
        doc.setFont(undefined, 'bold');
        doc.setFontSize(12);
        doc.text('INFRASTRUCTURE ASSESSMENT', margin + 2, currentY + 4);
        currentY += 18;
        
        doc.setFont(undefined, 'normal');
        doc.setFontSize(10);
        
        // Calculate totals and averages - only for amenities with data
        const amenitiesWithData = amenityCategories.filter(cat => (normalizedAmenities[cat.key] || 0) > 0);
        const tableTotalAmenities = amenitiesWithData.reduce((sum, cat) => sum + (normalizedAmenities[cat.key] || 0), 0);
        const averagePerCategory = amenitiesWithData.length > 0 ? tableTotalAmenities / amenitiesWithData.length : 0;
        
        // Assessment metrics in a professional grid layout
        const assessments = [
            { label: 'Total Nearby Amenities:', value: `${tableTotalAmenities} facilities`, icon: '🏢' },
            { label: 'Amenity Categories Found:', value: `${amenitiesWithData.length} types`, icon: '📊' },
            { label: 'Average per Category:', value: `${averagePerCategory.toFixed(1)} facilities`, icon: '📈' },
            { label: 'Accessibility Score:', value: this.getAccessibilityRating(normalizedAmenities), icon: '🚗' },
            { label: 'Market Potential:', value: this.getMarketPotential(normalizedAmenities), icon: '💰' },
            { label: 'Customer Convenience:', value: `${this.getConvenienceScore(normalizedAmenities)}/10`, icon: '⭐' }
        ];
        ...
        assessments.forEach((assessment, index) => {
            ...
            doc.setFillColor(255, 255, 255);
            doc.rect(x, y - 2, (contentWidth / 2) - 5, 12, 'F');
            doc.setDrawColor(240, 240, 240);
            doc.setLineWidth(0.3);
            doc.rect(x, y - 2, (contentWidth / 2) - 5, 12, 'S');
            
            // Label
            doc.setTextColor(0, 0, 0);
            doc.setFont(undefined, 'normal');
            doc.setFontSize(9);
            doc.text(assessment.label, x + 3, y + 4);
            
            // Value with emphasis
            doc.setFont(undefined, 'bold');
            doc.setTextColor(28, 102, 69);
            doc.setFontSize(10);
            doc.text(assessment.value, x + 3, y + 8);
    ...
    async createLandUseChart(landUse) {
        try {
            // Create canvas element with proper dimensions
            const canvas = document.createElement('canvas');
            canvas.width = 600;
            canvas.height = 400;
            canvas.style.position = 'absolute';
            ...
            // Prepare data for chart - ensure we have valid data
            const percentages = landUse.percentages || {};
            const counts = landUse.counts || {};
            
            // Combine percentages and counts data, prefer percentages if available
            const chartData = {};
            const allKeys = new Set([...Object.keys(percentages), ...Object.keys(counts)]);
            
            allKeys.forEach(key => {
                const percentage = parseFloat(percentages[key]) || 0;
                const count = parseInt(counts[key]) || 0;
                if (percentage > 0 || count > 0) {
                    chartData[key] = percentage > 0 ? percentage : count;
                }
            });
            ...
            if (labels.length === 0) {
                ...
                labels.push(...Object.keys(sampleData));
                data.push(...Object.values(sampleData));
            }

            // Color scheme for land use types
            const colors = {
                'commercial': '#FF6B6B',
                'residential': '#4ECDC4', 
                'industrial': '#45B7D1',
            ...
            const chart = new Chart(ctx, {
                ...
                options: {
                    ...
                    scales: {
                        y: {
                            beginAtZero: true,
                            max: Math.max(...data) > 100 ? Math.max(...data) * 1.1 : 100,
                            ticks: {
            ...
            console.error('❌ Error creating land use chart:', error);
            return null;
        } finally {
    ...
    async createAmenitiesChart(amenities) {
        try {
            ...
            // Prepare data for chart - use normalized amenities
            const amenityData = {};
            const amenityLabels = {
                'schools': 'Schools',
                'hospitals': 'Hospitals',
            ...
            // Only include amenities with counts > 0
            Object.keys(amenityLabels).forEach(key => {
                const value = amenities[key] || 0;
                if (value > 0) {
                    amenityData[amenityLabels[key]] = value;
                }
            });

            // If no real data, don't create a chart
            ...
            const sortedEntries = Object.entries(amenityData).sort((a, b) => b[1] - a[1]);
            const labels = sortedEntries.map(entry => entry[0]);
            const data = sortedEntries.map(entry => entry[1]);

            // Create gradient colors for bars
            const colors = labels.map((_, index) => {
                const intensity = 1 - (index / labels.length) * 0.5;
                return `rgba(28, 102, 69, ${intensity})`;
            });
            ...
            const chart = new Chart(ctx, {
                ...
                options: {
                    ...
                    scales: {
                        x: {
                            beginAtZero: true,
                            max: Math.max(...data) > 0 ? Math.max(...data) * 1.1 : 20,
                            ticks: {
                                stepSize: Math.ceil(Math.max(...data) / 8) || 1,
                                font: {
                                    size: 11
                                },
                                color: '#2C3E50'
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'Nearby Amenities Distribution',
                            font: {
                                size: 16,
                                weight: 'bold'
                            },
                        ...
                        tooltip: {
                            ...
                            callbacks: {
                                label: function(context) {
                                    return `${context.label}: ${context.parsed.x} facility(ies)`;
                                }
                            }
                        }
                    },
...
Focus on business insights, market positioning, and strategic value. Use petroleum industry terminology and PSO perspective, don't provide any negative points.`;
    }
    ...
    showAITypingUI() {
        ...
        // Create AI typing container with system-aligned design
        ...
        typingCard.innerHTML = `
            <!-- System-aligned Background Pattern -->
            <div style="
                position: absolute;
                top: 0;
                left: 0;
            ...
            <div style="position: relative; z-index: 2;">
                <!-- System-aligned Professional Header -->
                <div style="
                ...
                ">
                    ...
                    ">
                        ...
                        <div style="display: flex; align-items: center;">
                            ...
                            <div style="
                                ...
                                overflow: hidden;
                            ">
                                <img src="assets/logos/pso.png" alt="PSO" style="
                                    width: 90%;
                                    height: 90%;
                                    object-fit: contain;
                            ...
                            <div>
                                ...
                                ">AI Neural Analytics Engine</h3>
                                <div style="display: flex; align-items: center; margin-top: 4px;">
                                    <span style="
                                        color: #9ca3af;
                                        font-size: 12px;
                                        margin-right: 12px;
                                    ">Pakistan State Oil • Advanced Analytics</span>
                                    <span style="
                        ...
                        ">
                            ...
                            ">
                                <div style="display: flex; gap: 4px; margin-right: 8px;">
                                    <div class="neural-dot" style="
                                        width: 8px;
                                        height: 8px;
                                        background: #3b82f6;
                                        border-radius: 50%;
                                        animation: neuralProcess 1.5s infinite ease-in-out both;
                                    "></div>
                                    <div class="neural-dot" style="
                                        width: 8px;
                                        height: 8px;
                    ...
                    <div style="
                        width: 60px;
                        height: 60px;
                        background: linear-gradient(135deg, #00AA44, #00DD55, #00FF66);
                        border-radius: 50%;
                        display: flex;
                        align-items: center;
                        justify-content: center;
                    ...
                    <div style="flex: 1;">
                        <h3 style="
                            ...
                            font-size: 24px; 
                            font-weight: bold;
                            background: linear-gradient(45deg, #00DD55, #00FF66, #33FF88);
                            -webkit-background-clip: text;
                            -webkit-text-fill-color: transparent;
                            background-clip: text;
                            text-shadow: 0 0 10px rgba(0, 221, 85, 0.3);
                        ">PSO AI Neural Analytics Engine</h3>
                        <p style="
                            margin: 5px 0 0; 
                            opacity: 0.8; 
                            font-size: 14px;
                        ...
                        ">🚀 Powered by Advanced AI • Neural Processing Active • Quantum Analysis Mode</p>
                    </div>
                    <div id="aiTypingIndicator" style="
                        display: flex;
                        align-items: center;
                    ...
                    ">
                        <div class="neural-dot" style="
                            ...
                            background: #00DD55;
                            border-radius: 50%;
                            animation: neuralPulse 1.2s infinite ease-in-out both;
                            box-shadow: 0 0 10px #00DD55;
                        "></div>
                        <div class="neural-dot" style="
                            width: 10px;
                            height: 10px;
                ...
                ">
                    ...
                    <div id="aiReportContent" style="
                        ...
                        scrollbar-color: rgba(0, 221, 85, 0.5) rgba(0, 0, 0, 0.3);
                    "></div>
                </div>
                ...
                ">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 15px;">
                        <span id="progressText" style="
                            font-size: 14px; 
                            opacity: 0.9;
                            color: #d1d5db;
                            font-weight: 500;
                        ">🔄 Initializing neural networks...</span>
        ...
        style.textContent = `
            ...
            #aiReportContent::-webkit-scrollbar {
                width: 8px;
            }
            
            #aiReportContent::-webkit-scrollbar-track {
                background: rgba(17, 24, 39, 0.6);
                border-radius: 4px;
            }
            
            #aiReportContent::-webkit-scrollbar-thumb {
                background: rgba(59, 130, 246, 0.6);
                border-radius: 4px;
            }
            
            #aiReportContent::-webkit-scrollbar-thumb:hover {
                background: rgba(59, 130, 246, 0.8);
            }
            
            /* Smooth text selection */
            #aiReportContent::selection {
    ...
    async simulateAIReportGeneration(allData) {
        ...
        // Show AI processing message (ultra fast)
        progressText.textContent = '🧠 Connecting to PSO Neural Network...';
        await this.typeText(contentElement, '� PSO AI Neural Analytics Engine Initializing...\n\n', 2); // 2ms vs 5ms
        await this.sleep(100); // 100ms vs 200ms

        // Generate AI-enhanced report sections
        progressText.textContent = '⚡ AI processing your data at quantum speed...';
        await this.typeText(contentElement, '� Processing site data with advanced neural algorithms...\n', 2); // 2ms vs 5ms
        ...
        await this.typeText(contentElement, '✅ Neural analysis complete. Generating quantum insights...\n\n', 2); // 2ms vs 5ms
        ...
        for (let i = 0; i < reportSections.length; i++) {
            ...
            // Add section header (ultra fast)
            await this.typeText(contentElement, `\n\n🔸 ${section.title}\n${'▔'.repeat(section.title.length + 2)}\n\n`, 1); // 1ms vs 3ms
            
            // Type section content word by word (ultra fast)
            ...
            for (let j = 0; j < words.length; j++) {
                ...
                await this.typeText(contentElement, textToAdd, Math.random() * 4 + 2); // Ultra fast: 2-6ms vs 5-13ms
                ...
                // Even less frequent pauses for ultra speed
                if (Math.random() < 0.005) { // Only 0.5% chance vs 1%
                    await this.sleep(Math.random() * 10 + 5); // 5-15ms vs 10-30ms
                }
            }
            
            // Ultra short pause between sections
            await this.sleep(50); // 50ms vs 75ms
        }

        // Final completion
        progressText.textContent = '🎯 Neural report generation complete!';
    ...
    // ========================================
    // HELPER METHODS FOR LAND USE AND AMENITIES ANALYSIS
    // ========================================

    getDominantLandUseColor(landUse) {
        const colors = {
            'Commercial': [28, 102, 69],
            'Residential': [52, 152, 219],
            'Industrial': [155, 89, 182],
            'Mixed Use': [230, 126, 34],
            'Retail': [231, 76, 60],
            'Office': [52, 73, 94],
            'Mixed': [230, 126, 34]
        };
        return colors[landUse] || [108, 117, 125];
    }

    getZoningCompatibility(landUse) {
        const percentages = landUse.percentages || {};
        const commercial = parseFloat(percentages.commercial) || 0;
        const mixed = parseFloat(percentages.mixed) || 0;
        const retail = parseFloat(percentages.retail) || 0;
        
        const compatibilityScore = commercial + mixed + (retail * 0.8);
        
        if (compatibilityScore >= 60) return 'Highly Compatible';
        if (compatibilityScore >= 40) return 'Moderately Compatible';
        if (compatibilityScore >= 20) return 'Limited Compatibility';
        return 'Low Compatibility';
    }

    getDevelopmentPotential(landUse) {
        const percentages = landUse.percentages || {};
        const commercial = parseFloat(percentages.commercial) || 0;
        const mixed = parseFloat(percentages.mixed) || 0;
        const industrial = parseFloat(percentages.industrial) || 0;
        
        const developmentScore = commercial + mixed + (industrial * 0.6);
        
        if (developmentScore >= 50) return 'High Potential';
        if (developmentScore >= 30) return 'Medium Potential';
        if (developmentScore >= 15) return 'Limited Potential';
        return 'Low Potential';
    }

    getTrafficGeneration(landUse) {
        const percentages = landUse.percentages || {};
        const commercial = parseFloat(percentages.commercial) || 0;
        const retail = parseFloat(percentages.retail) || 0;
        const office = parseFloat(percentages.office) || 0;
        const mixed = parseFloat(percentages.mixed) || 0;
        
        const trafficScore = commercial + retail + office + mixed;
        
        if (trafficScore >= 60) return 'High Traffic Generation';
        if (trafficScore >= 40) return 'Medium Traffic Generation';
        if (trafficScore >= 20) return 'Moderate Traffic Generation';
        return 'Low Traffic Generation';
    }

    getAmenityImpact(amenityType, count) {
        const impactMap = {
            'schools': { low: 2, medium: 5, high: 10 },
            'hospitals': { low: 1, medium: 3, high: 6 },
            'restaurants': { low: 5, medium: 15, high: 30 },
            'malls': { low: 1, medium: 2, high: 4 },
            'banks': { low: 2, medium: 5, high: 10 },
            'offices': { low: 3, medium: 8, high: 15 },
            'hotels': { low: 1, medium: 3, high: 6 },
            'parks': { low: 1, medium: 3, high: 6 }
        };

        const thresholds = impactMap[amenityType] || { low: 1, medium: 3, high: 6 };
    ...
    calculateInfrastructureScore(amenities) {
        let score = 0;
        const weights = {
            'schools': 15,
            'hospitals': 20,
        ...
        Object.keys(weights).forEach(amenity => {
            const count = parseInt(amenities[amenity]) || 0;
            const weight = weights[amenity];
            
            // Normalize count to a 0-1 scale (max 10 facilities per type)
            const normalizedCount = Math.min(count / 10, 1);
    ...
    getAmenityDensity(amenities) {
        const totalAmenities = Object.values(amenities).reduce((sum, count) => {
            return sum + (parseInt(count) || 0);
        }, 0);
        
        if (totalAmenities >= 50) return 'Very High';
        if (totalAmenities >= 30) return 'High';
        if (totalAmenities >= 15) return 'Medium';
        if (totalAmenities >= 5) return 'Low';
        return 'Very Low';
    }

    getCustomerAttractionPotential(amenities) {
        const highTrafficAmenities = ['restaurants', 'malls', 'schools', 'offices'];
        const highTrafficCount = highTrafficAmenities.reduce((sum, amenity) => {
            return sum + (parseInt(amenities[amenity]) || 0);
        }, 0);
        
        if (highTrafficCount >= 25) return 'Excellent';
        if (highTrafficCount >= 15) return 'Good';
        if (highTrafficCount >= 8) return 'Moderate';
        if (highTrafficCount >= 3) return 'Limited';
        return 'Poor';
    }

    getBusinessSynergy(amenities) {
        const synergyPairs = [
            ['malls', 'restaurants'],
            ['offices', 'banks'],
            ['hotels', 'restaurants'],
        ...
        synergyPairs.forEach(pair => {
            const [amenity1, amenity2] = pair;
            const count1 = parseInt(amenities[amenity1]) || 0;
            const count2 = parseInt(amenities[amenity2]) || 0;
            if (count1 > 0 && count2 > 0) {
                synergyScore += Math.min(count1 + count2, 10);
            }
        });

        if (synergyScore >= 30) return 'Excellent Synergy';
        if (synergyScore >= 20) return 'Good Synergy';
        if (synergyScore >= 10) return 'Moderate Synergy';
        if (synergyScore >= 5) return 'Limited Synergy';
        return 'Poor Synergy';
    }

    getAccessibilityRating(amenities) {
        const totalAmenities = Object.values(amenities).reduce((sum, count) => sum + (parseInt(count) || 0), 0);
        const publicTransport = parseInt(amenities.public_transport) || 0;
        const parking = parseInt(amenities.parking) || 0;
        
        let rating = 'Poor';
        let score = totalAmenities + (publicTransport * 2) + (parking * 1.5);
        
        if (score >= 50) rating = 'Excellent';
        else if (score >= 35) rating = 'Good';
        else if (score >= 20) rating = 'Fair';
        
        return rating;
    }

    getMarketPotential(amenities) {
        const highValueAmenities = ['malls', 'offices', 'hotels', 'hospitals'];
        const highValueCount = highValueAmenities.reduce((sum, type) => sum + (parseInt(amenities[type]) || 0), 0);
        
        if (highValueCount >= 15) return 'High';
        if (highValueCount >= 8) return 'Medium';
        if (highValueCount >= 3) return 'Moderate';
        return 'Low';
    }

    getCompetitionLevel(amenities) {
        const gasStations = parseInt(amenities.gas_stations) || 0;
        
        if (gasStations >= 5) return 'High Competition';
        if (gasStations >= 3) return 'Medium Competition';
        if (gasStations >= 1) return 'Low Competition';
        return 'No Direct Competition';
    }

    getConvenienceScore(amenities) {
        const convenienceFactors = ['restaurants', 'banks', 'malls', 'parking'];
        const convenienceTotal = convenienceFactors.reduce((sum, type) => sum + (parseInt(amenities[type]) || 0), 0);
        
        // Score out of 10
        return Math.min(Math.round(convenienceTotal / 5), 10);
    }
}
